...
class CRMessengerAuthorizationClient(interface.ICRMessengerAuthorizationClient):
    def __init__(
            self,
    ...
    async def authorization(self, account_id: int, two_fa_status: bool) -> model.JWTTokens:
        with self.tracer.start_as_current_span(
                "CRMessengerAuthorizationClient.authorization",
    ...
    async def check_authorization(self, access_token: str) -> model.AuthorizationData:
        with self.tracer.start_as_current_span(
                "CRMessengerAuthorizationClient.check_authorization",
                kind=SpanKind.CLIENT,
        ) as span:
            try:
