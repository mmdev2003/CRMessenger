...
class HttpMiddleware(interface.IHttpMiddleware):
        ...
        async def _trace_middleware01(request: Request, call_next: Callable):
            ...
            with self.tracer.start_as_current_span(
                ...
                try:
                    ...
                    if status_code >= 500:
                        root_span.set_status(Status(StatusCode.ERROR, "Internal server error"))
                        root_span.set_attribute(common.ERROR_KEY, True)
                    elif status_code >= 400:
                        root_span.set_status(Status(StatusCode.ERROR, "Client error"))
                        root_span.set_attribute(common.ERROR_KEY, True)
                    else:
    ...
    def metrics_middleware02(self, app: FastAPI):
        ...
        async def _metrics_middleware02(request: Request, call_next: Callable):
            ...
            try:
                ...
                request_attrs[common.ERROR_KEY] = str(e)
    ...
    def logger_middleware03(self, app: FastAPI):
        ...
        async def _logger_middleware03(request: Request, call_next: Callable):
            ...
            try:
                self.logger.info("Началась обработка запроса", extra_log)
                response = await call_next(request)
                ...
                self.logger.info("Обработка запроса завершена успешно", extra_log)
                ...
                extra_log = {
                    **extra_log,
                    ...
                    common.ERROR_KEY: str(e),
                }

                self.logger.error(f"Обработка запроса завершена с ошибкой: {str(e)}", extra_log)
                raise
            ...
            with self.tracer.start_as_current_span(
                ...
                try:
                    ...
                    if not access_token:
                        ...
                        authorization_data = await crmessenger_authorization_client.check_authorization(access_token)
